...
async function loadThreadCollection(){
	...
	try{
		...
		return client.db('Cluster0').collection('threads');
	} catch (err) {
...
router.get('/', async (req, res) => {
	let posts = await loadThreadCollection();
	res.send(await posts.find({}).toArray());
});

//Search
router.get('/search', async (req, res) => {
	const posts = await loadThreadCollection();
